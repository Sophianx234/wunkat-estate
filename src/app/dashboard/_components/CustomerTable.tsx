...
export default function CustomersTable({ data }: Props) {
  ...
  const [rentFilter, setRentFilter] = useState<"all" | "active" | "due_soon" | "expired">("all");
    ...
    <Card className="border-none shadow-none">
      <CardHeader className="pb-0">
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-sm text-gray-500">Manage and monitor all tenants</p>
          </div>
      ...
      <CardContent className="mt-6 space-y-4">
        {table.getRowModel().rows.map((row) => (
          <div key={row.id}>{flexRender(row.getVisibleCells()[0].column.columnDef.cell, row.getVisibleCells()[0].getContext())}</div>
        ))}
